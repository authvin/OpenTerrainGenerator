...
public final class CoralHelper
{
    ...
    public static boolean placeCoralBlock(
        ...
        )
        {
            ...
            for (OTGDirection direction : HORIZONTAL) {
                if (random.nextFloat() < 0.2f) {
                    if (world.getMaterial(x + direction.getX(), y + direction.getY(), z + direction.getZ())
                             .isMaterial(LocalMaterials.WATER))
                    {
                        wallMaterial =
                            getRandomWallCoral(random).withProperty(MaterialProperties.HORIZONTAL_DIRECTION, direction);
