...
class Vein
{
    ...
    public boolean reachesChunk(int otherChunkX, int otherChunkZ)
    {
        ...
        // TODO: Is this the decoration offset? If so, remove it?
        int chunkX = (x + 8) / 16;
        int chunkZ = (z + 8) / 16;
    ...
    public void spawn(IWorldGenRegion worldGenRegion, Random random, VeinResource gen)
    {
        ...
        for (int i = 0; i < gen.oreFrequency; i++) {
            if (random.nextInt(100) < gen.oreRarity) {
                oreX = worldGenRegion.getDecorationArea().getChunkBeingDecoratedMinX()
                       + random.nextInt(Constants.CHUNK_SIZE);
                oreY = RandomHelper.numberInRange(random, gen.minAltitude, gen.maxAltitude);
                oreZ = worldGenRegion.getDecorationArea().getChunkBeingDecoratedMinZ()
                       + random.nextInt(Constants.CHUNK_SIZE);
                        ...
                        if (d13 * d13 + d14 * d14 < 1.0D) {
                            for (int i5 = m; i5 <= i2; i5++) {
                                ...
                                if ((d13 * d13 + d14 * d14 + d15 * d15 < 1.0D)
                                    && sourceBlocks.contains(worldGenRegion.getMaterial(i3, i4, i5)))
                                {
