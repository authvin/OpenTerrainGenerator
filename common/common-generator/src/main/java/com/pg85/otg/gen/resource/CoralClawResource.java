...
public class CoralClawResource extends FrequencyResourceBase
{
    ...
    public CoralClawResource(BiomeSettings biomeConfig, List<String> args) throws InvalidConfigException
    {
    ...
    public void spawn(IWorldGenRegion world, Random random, int x, int z)
    {
        ...
        if (y < world.getWorldInfo().minY() ||
            ...
            !CoralHelper.placeCoralBlock(world, random, x, y, z, coral)
        )
        {
            return;
        }
        ...
        for (OTGDirection direction : list.subList(0, dirEnd)) {
            ...
            for (int i = 0;
                 i < branchLength
                 && dy >= world.getWorldInfo().minY()
                 && dy <= world.getWorldInfo().maxY()
                 && CoralHelper.placeCoralBlock(world, random, dx, dy, dz, coral);
                 ++i
            ) {
                dx += finalDir.getX();
                dy += finalDir.getY();
