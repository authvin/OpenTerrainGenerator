...
{	
	...
	public BO4CustomStructure(long worldSeed, BO4CustomStructureCoordinate structureStart, Map<ChunkCoordinate, Stack<BO4CustomStructureCoordinate>> objectsToSpawn, Map<ChunkCoordinate, ArrayList<SmoothingAreaLine>> smoothingAreasToSpawn, int minY, Path otgRootFolder)
	{
		this(worldSeed, structureStart, otgRootFolder);
		this.objectsToSpawn = objectsToSpawn;
		this.smoothingAreaManager.fillSmoothingLineCaches(smoothingAreasToSpawn);
		this.minY = minY;
	}
	
	public BO4CustomStructure(long worldSeed, BO4CustomStructureCoordinate start, Path otgRootFolder)
	{
		...
		if (!(start.getObject(otgRootFolder) instanceof StructuredCustomObject))
		{
	...
	BO4CustomStructure(CustomStructureCache structureCache, IWorldGenRegion worldGenRegion, BO4CustomStructureCoordinate start, boolean isStructureAtSpawn, boolean ignoreSpawnSettings, ArrayList<String> targetBiomes, ChunkCoordinate chunkBeingDecorated, Path otgRootFolder,  CustomObjectManager customObjectManager, IMaterialReader materialReader, CustomObjectResourcesManager manager, IModLoadedChecker modLoadedChecker)
	{
		...
		try
		{
			BO4Config bo4Config = ((BO4)this.start.getObject(otgRootFolder)).getConfig();
			calculateBranches(bo4Config, false, structureCache, worldGenRegion, targetBiomes, chunkBeingDecorated, otgRootFolder);
		...
		for(Entry<ChunkCoordinate, Stack<BO4CustomStructureCoordinate>> chunkCoordSet : this.objectsToSpawn.entrySet())
		{
			if(chunkCoordSet.getValue() != null)
			{
				...
				{
					structureInfo.append(customObjectCoord.getObject(otgRootFolder).getName()).append(":").append(customObjectCoord.getRotation()).append(", ");
				}
		...
		for(Entry<ChunkCoordinate, Stack<BO4CustomStructureCoordinate>> chunkCoordSet : this.objectsToSpawn.entrySet())
		{
			if(chunkCoordSet.getValue() != null)
			{
				...
				{
					BO4Config objectConfig = ((BO4)coordObject.getObject(otgRootFolder)).getConfig();
					if(!objectConfig.replacesBO3Branches.isEmpty())
					{
						for(String BO3ToReplace : objectConfig.replacesBO3Branches)
						{
							...
							{
								if(((BO4)coordObjectToReplace.getObject(otgRootFolder)).getName().equalsIgnoreCase(BO3ToReplace))
								{
									if(checkCollision(coordObject, coordObjectToReplace, otgRootFolder))
									{
		...
		if(!this.objectsToSpawn.isEmpty())
		{
			...
			if(OTGLog.getLogCategoryEnabled(LogCategory.STRUCTURE_PLOTTING))
			{
				...
				OTGLog.log(LogLevel.INFO, LogCategory.STRUCTURE_PLOTTING, this.start.getObject(otgRootFolder).getName() + " " + totalBO3sSpawned + " object(s) plotted in " + (System.currentTimeMillis() - startTime) + " Ms and " + Cycle + " cycle(s), " + (branchesTried + 1) + " object(s) tried.");
			}
		}
	}
	...
	private boolean doStartChunkBlockChecks(IWorldGenRegion worldGenRegion, ChunkCoordinate chunkBeingDecorated, Path otgRootFolder,  CustomObjectManager customObjectManager, IMaterialReader materialReader, CustomObjectResourcesManager manager, IModLoadedChecker modLoadedChecker, boolean ignoreSpawnSettings)
	{
		...
		if(!startChunkBlockChecksDone)
		{
			...
			BO4Config config = ((BO4)this.start.getObject(otgRootFolder)).getConfig();
	 ...
	 */
	public Object[] getMinimumSize(CustomStructureCache structureCache, IWorldGenRegion worldGenRegion, Path otgRootFolder) throws InvalidConfigException
	{
		BO4 bo4 = ((BO4)this.start.getObject(otgRootFolder));
		...
		calculateBranches(bo4Config, true, structureCache, worldGenRegion, null, null, otgRootFolder);
	...
	private void calculateBranches(BO4Config startBO4Config, boolean minimumSize, CustomStructureCache structureCache, IWorldGenRegion worldGenRegion, ArrayList<String> targetBiomes, ChunkCoordinate chunkBeingDecorated, Path otgRootFolder) throws InvalidConfigException
	{
		if(OTGLog.getLogCategoryEnabled(LogCategory.STRUCTURE_PLOTTING))
		{
		...
		if(OTGLog.getLogCategoryEnabled(LogCategory.STRUCTURE_PLOTTING))
		{
			...
			OTGLog.log(LogLevel.INFO, LogCategory.STRUCTURE_PLOTTING, "Plotted X" + branchData.chunkCoordinate.getChunkX() + " Z" + branchData.chunkCoordinate.getChunkZ() + " - " + branchData.branch.getObject(otgRootFolder).getName());
		}

		addToCaches(branchData, ((BO4)branchData.branch.getObject(otgRootFolder)));
		...
		while(!processingDone)
		{
			...
			traverseAndSpawnChildBranches(startBO4Config, branchData, minimumSize, true, structureCache, worldGenRegion, targetBiomes, chunkBeingDecorated, otgRootFolder);
			...
			traverseAndSpawnChildBranches(startBO4Config, branchData, minimumSize, false, structureCache, worldGenRegion, targetBiomes, chunkBeingDecorated, otgRootFolder);
			...
			if(processingDone && !canOverrideBranchesSpawned)
			{
				...
				for(BranchDataItem branchDataItem3 : AllBranchesBranchData)
				{
					for(BranchDataItem childBranch : branchDataItem3.getChildren(false, worldGenRegion, chunkBeingDecorated, otgRootFolder))
					{
						if(
							!childBranch.branch.isRequiredBranch &&
							((BO4)childBranch.branch.getObject(otgRootFolder)).getConfig().canOverride
						)
						{
	...
	private void traverseAndSpawnChildBranches(BO4Config startBO4Config, BranchDataItem branchData, boolean minimumSize, boolean spawningRequiredBranchesOnly, CustomStructureCache structureCache, IWorldGenRegion worldGenRegion, ArrayList<String> targetBiomes, ChunkCoordinate chunkBeingDecorated, Path otgRootFolder)
	{
		if(!branchData.doneSpawning)
		{
			addBranches(startBO4Config, branchData, minimumSize, false, spawningRequiredBranchesOnly, structureCache, worldGenRegion, targetBiomes, chunkBeingDecorated, otgRootFolder);
		} else {
			if(!branchData.cannotSpawn)
			{
				for(BranchDataItem branchDataItem2 : branchData.getChildren(false, worldGenRegion, chunkBeingDecorated, otgRootFolder))
				{
					...
					{
						traverseAndSpawnChildBranches(startBO4Config, branchDataItem2, minimumSize, spawningRequiredBranchesOnly, structureCache, worldGenRegion, targetBiomes, chunkBeingDecorated, otgRootFolder);
					}
				}
			}
		}
	...
	private void addBranches(BO4Config startBO4Config, BranchDataItem branchDataItem, boolean minimumSize, boolean traverseOnlySpawnedChildren, boolean spawningRequiredBranchesOnly, CustomStructureCache structureCache, IWorldGenRegion worldGenRegion, ArrayList<String> targetBiomes, ChunkCoordinate chunkBeingDecorated, Path otgRootFolder)
	{
		...
		{
			for(BranchDataItem branchDataItem3 : branchDataItem.getChildren(false, worldGenRegion, chunkBeingDecorated, otgRootFolder))
			{
				if(
					(
						...
						((BO4)branchDataItem3.branch.getObject(otgRootFolder)).getConfig().canOverride &&
		...
		{			
			for(BranchDataItem childBranchDataItem : branchDataItem.getChildren(false, worldGenRegion, chunkBeingDecorated, otgRootFolder))
			{			
				if(!AllBranchesBranchDataHash.contains(childBranchDataItem.branchNumber) && !childBranchDataItem.spawnDelayed)
				{
					...
					BO4 bo4 = ((BO4)childBranchDataItem.branch.getObject(otgRootFolder));
					...
					if(canSpawn)
					{
						...
						{
							canSpawn = checkMustBeBelowOther(childBranchDataItem, otgRootFolder);
							if(!canSpawn)
							{
						...
						{							
							canSpawn = checkMustBeInside(childBranchDataItem, bo4, otgRootFolder);
							if(!canSpawn)
							{
						...
						{
							canSpawn = checkCannotBeInside(childBranchDataItem, bo4, otgRootFolder);
							if(!canSpawn)
							{
						...
						{
							if(!checkYBounds(worldGenRegion.getPresetConfig().getBlockSettings().isDisableBedrock(), childBranchDataItem.branch, otgRootFolder))
							{
								canSpawn = false;
								chunkIsIneligible = true;
							}
						...
						if(canSpawn)
						{
							...
							collidingBranches = checkSpawnRequirementsAndCollisions(startBO4Config, structureCache, childBranchDataItem, minimumSize, worldGenRegion, targetBiomes, otgRootFolder);
					...
					if(canSpawn)
					{
						...
						if(childBranchDataItem.getChildren(false, worldGenRegion, chunkBeingDecorated, otgRootFolder).isEmpty())
						{
							childBranchDataItem.doneSpawning = true;
						}

						// Mark any branches in the same branch group so they wont try to spawn
						for(BranchDataItem childBranchDataItem2 : branchDataItem.getChildren(false, worldGenRegion, chunkBeingDecorated, otgRootFolder))
						{
							if(
						...
						)
						{
							...
							traverseAndSpawnChildBranches(startBO4Config, childBranchDataItem, minimumSize, true, structureCache, worldGenRegion, targetBiomes, chunkBeingDecorated, otgRootFolder);
					...
					if(!canSpawn)
					{
						if(!childBranchDataItem.doneSpawning && !childBranchDataItem.cannotSpawn)
						{
							...
							if((!wasntBelowOther || !spawnedBranchLastCycle) && branchGroupFailedSpawning)
							{
								...
								rollBackBranch(startBO4Config, branchDataItem, minimumSize, spawningRequiredBranchesOnly, structureCache, worldGenRegion, targetBiomes, chunkBeingDecorated, otgRootFolder);
								bBreak = true;
							} else {
								...
								for(BranchDataItem childBranchDataItem2 : branchDataItem.getChildren(false, worldGenRegion, chunkBeingDecorated, otgRootFolder))
								{
									if(!wasntBelowOther || !spawnedBranchLastCycle)
									{
										BO4Config childBranchDataItem2Config = ((BO4)childBranchDataItem2.branch.getObject(otgRootFolder)).getConfig();
										if(
										...
										)
										{
											...
											if(childBranchDataItem2.branch.isRequiredBranch)
											{
												...
												// Check if there are any more branches in this group that haven't tried to spawn yet.
												// *At this point, all optional branches should have had a chance to spawn.
												for(BranchDataItem childBranchDataItem3 : branchDataItem.getChildren(false, worldGenRegion, chunkBeingDecorated, otgRootFolder))
												{
													if(
			...
			)
			{
				for(BranchDataItem childBranchDataItem : branchDataItem.getChildren(false, worldGenRegion, chunkBeingDecorated, otgRootFolder))
				{
					if(AllBranchesBranchDataHash.contains(childBranchDataItem.branchNumber))
					{
						if(
							(
								...
								(
									...
									!((BO4)childBranchDataItem.branch.getObject(otgRootFolder)).getConfig().canOverride
								)
							) &&
						...
						{
							traverseAndSpawnChildBranches(startBO4Config, childBranchDataItem, minimumSize, false, structureCache, worldGenRegion, targetBiomes, chunkBeingDecorated, otgRootFolder);
						}
					}
				}
			}
	...
	private boolean checkMustBeBelowOther(BranchDataItem childBranchDataItem, Path otgRootFolder)
	{
		...
		if(AllBranchesBranchDataByChunk.containsKey(childBranchDataItem.chunkCoordinate))
		{
			for(BranchDataItem branchDataItem2 : AllBranchesBranchDataByChunk.get(childBranchDataItem.chunkCoordinate))
			{
				if(
					...
					!((BO4) branchDataItem2.branch.getObject(otgRootFolder)).getConfig().canOverride &&
	...
	private boolean checkCannotBeInside(BranchDataItem childBranchDataItem, BO4 bo3, Path otgRootFolder)
	{
		...
		if(AllBranchesBranchDataByChunk.containsKey(childBranchDataItem.chunkCoordinate))
		{
			...
			for(String cantBeInsideBO3 : bo3.getConfig().cannotBeInsideBranches)
			{
				for(BranchDataItem branchDataItem3 : branchDataInChunk)
				{
					...
					{
						for(String branchName : ((BO4)branchDataItem3.branch.getObject(otgRootFolder)).getConfig().getInheritedBO3s()) // getInheritedBO3s also contains this BO3
						{
							if(branchName.equalsIgnoreCase(cantBeInsideBO3))
							{
									if(checkCollision(childBranchDataItem.branch, branchDataItem3.branch, otgRootFolder))
									{
			 							if(OTGLog.getLogCategoryEnabled(LogCategory.STRUCTURE_PLOTTING))
										{
	...
	private boolean checkMustBeInside(BranchDataItem childBranchDataItem, BO4 bo3, Path otgRootFolder)
	{
	...
	private void rollBackBranch(BO4Config startBO4Config, BranchDataItem branchData, boolean minimumSize, boolean spawningRequiredBranchesOnly, CustomStructureCache structureCache, IWorldGenRegion worldGenRegion, ArrayList<String> targetBiomes, ChunkCoordinate chunkBeingDecorated, Path otgRootFolder)
	{
		...
		deleteBranchChildren(startBO4Config, branchData, minimumSize, spawningRequiredBranchesOnly, structureCache, worldGenRegion, targetBiomes, chunkBeingDecorated, otgRootFolder);
		...
		if(AllBranchesBranchDataHash.contains(branchData.branchNumber))
		{
			...
			removeFromCaches(branchData, otgRootFolder);
		}

		BO4Config branchDataConfig = ((BO4)branchData.branch.getObject(otgRootFolder)).getConfig();
			...
			if(branchDataByChunk != null)
			{
				...
				for(BranchDataItem branchDataItem2 : allBranchesBranchData2)
				{
					if(AllBranchesBranchDataHash.contains(branchDataItem2.branchNumber))
					{
						...
						{
							if(((BO4)branchDataItem2.branch.getObject(otgRootFolder)).getConfig().mustBeBelowOther && branchDataItem2.chunkCoordinate.equals(branchData.chunkCoordinate))
							{
								...
								{
									BO4Config branchDataItem3Config = ((BO4)branchDataItem3.branch.getObject(otgRootFolder)).getConfig();
									if(
								...
								if(!branchAboveFound)
								{
									rollBackBranch(startBO4Config, branchDataItem2, minimumSize, spawningRequiredBranchesOnly, structureCache, worldGenRegion, targetBiomes, chunkBeingDecorated, otgRootFolder);
								}
							}
						}
					}
		...
		if(branchDataByChunk != null)
		{
			...
			for(BranchDataItem branchDataItem2 : allBranchesBranchData2)
			{
				if(AllBranchesBranchDataHash.contains(branchDataItem2.branchNumber))
				{
					if(branchDataItem2 != branchData)
					{
						BO4 branchDataItem2BO4 = ((BO4)branchDataItem2.branch.getObject(otgRootFolder));
						...
						)
						{
							...
							if(currentBO3Found) 
							{
								...
								if(!checkMustBeInside(branchDataItem2, branchDataItem2BO4, otgRootFolder))
								{
									rollBackBranch(startBO4Config, branchDataItem2, minimumSize, spawningRequiredBranchesOnly, structureCache, worldGenRegion, targetBiomes, chunkBeingDecorated, otgRootFolder);
								}
							}
		...
		if(branchData.parent != null && !branchData.parent.isBeingRolledBack)
		{
			if(branchData.branch.isRequiredBranch)
			{
				...
				rollBackBranch(startBO4Config, branchData.parent, minimumSize, spawningRequiredBranchesOnly, structureCache, worldGenRegion, targetBiomes, chunkBeingDecorated, otgRootFolder);
			} else {
				...
				for (BranchDataItem branchDataItem2 : branchData.parent.getChildren(false, worldGenRegion, chunkBeingDecorated, otgRootFolder))
				{
				...
				)
				{
					...
					if(!spawningRequiredChildrenForOptionalBranch)
					{
						if(spawningRequiredBranchesOnly)
						{
							...
							addBranches(startBO4Config, branchData.parent, minimumSize, false, true, structureCache, worldGenRegion, targetBiomes, chunkBeingDecorated, otgRootFolder);
						} else {
							...
							addBranches(startBO4Config, branchData.parent, minimumSize, true, false, structureCache, worldGenRegion, targetBiomes, chunkBeingDecorated, otgRootFolder);
						}
					} else {
	...
	private void deleteBranchChildren(BO4Config startBO4Config, BranchDataItem branchData, boolean minimumSize, boolean spawningRequiredBranchesOnly, CustomStructureCache structureCache, IWorldGenRegion worldGenRegion, ArrayList<String> targetBiomes, ChunkCoordinate chunkBeingDecorated, Path otgRootFolder)
	{
		// Remove all children of this branch from AllBranchesBranchData
		Stack<BranchDataItem> children = branchData.getChildren(true, worldGenRegion, chunkBeingDecorated, otgRootFolder);
		for(BranchDataItem branchDataItem : children)
		{
			...
			if(!branchDataItem.getChildren(true, worldGenRegion, chunkBeingDecorated, otgRootFolder).isEmpty())
			{
				deleteBranchChildren(startBO4Config, branchDataItem, minimumSize, spawningRequiredBranchesOnly, structureCache, worldGenRegion, targetBiomes, chunkBeingDecorated, otgRootFolder);
			}
			if(AllBranchesBranchDataHash.contains(branchDataItem.branchNumber))
			{
				...
				removeFromCaches(branchDataItem, otgRootFolder);

				BO4Config branchDataItemConfig = ((BO4)branchDataItem.branch.getObject(otgRootFolder)).getConfig();
					...
					if(branchDataByChunk != null)
					{
						...
						for(BranchDataItem branchDataItem2 : allBranchesBranchData2)
						{
							if(AllBranchesBranchDataHash.contains(branchDataItem2.branchNumber))
							{
								...
								{
									if(((BO4)branchDataItem2.branch.getObject(otgRootFolder)).getConfig().mustBeBelowOther && branchDataItem2.chunkCoordinate.equals(branchDataItem.chunkCoordinate))
									{
										boolean branchAboveFound = false;
										for(BranchDataItem branchDataItem3 : AllBranchesBranchDataByChunk.get(branchDataItem2.chunkCoordinate))
										{
	...
	private void removeFromCaches(BranchDataItem branchDataItem, Path otgRootFolder)
	{		
	...
	// Returns null if the branch cannot spawn in the given biome or if there's another BO4 structure in the chunk, otherwise returns colliding branches. 
	// CanOverride branches never collide with other branches, but may be unable to spawn if there's not enough space for smoothing areas.
	private Stack<BranchDataItem> checkSpawnRequirementsAndCollisions(BO4Config startBO4Config, CustomStructureCache structureCache, BranchDataItem branchData, boolean minimumSize, IWorldGenRegion worldGenRegion, ArrayList<String> targetBiomes, Path otgRootFolder)
	{
		...
		if(!minimumSize)
		{
			...
			if(smoothRadius == -1 || ((BO4)coordObject.getObject(otgRootFolder)).getConfig().smoothRadius == -1)
			{
				smoothRadius = 0;
			}
			if(smoothRadius > 0)
		...
		if(!((BO4) coordObject.getObject(otgRootFolder)).getConfig().canOverride)
		{
			...
			if(AllBranchesBranchDataByChunk.containsKey(branchData.chunkCoordinate))
			{
				...
				{
					if(branchData.chunkCoordinate.equals(existingBranchData.chunkCoordinate) && !((BO4)existingBranchData.branch.getObject(otgRootFolder)).getConfig().canOverride)
					{
			...
			if (!existingBranches.isEmpty())
			{
				...
				{
					if(checkCollision(coordObject, cachedBranch.branch, otgRootFolder))
					{
	...
	private boolean checkYBounds(boolean disableBedrock, CustomStructureCoordinate branchData1Branch, Path otgRootFolder)
	{
		int startY = branchData1Branch.getY() + ((BO4)branchData1Branch.getObject(otgRootFolder)).getConfig().getminY();
		return disableBedrock ? startY >= 0 : startY > 0;
	}
	
	private boolean checkCollision(CustomStructureCoordinate branchData1Branch, CustomStructureCoordinate branchData2Branch, Path otgRootFolder)
	{
		BO4 branch1Object = ((BO4)branchData1Branch.getObject(otgRootFolder));
		BO4 branch2Object = ((BO4)branchData2Branch.getObject(otgRootFolder));
	...
	*/
	void spawnInChunk(ChunkCoordinate chunkCoordinate, CustomStructureCache structureCache, IWorldGenRegion worldGenRegion, Path otgRootFolder)
	{		
		if (
		...
		if (objectsInChunk != null)
		{
			...
			this.smoothingAreaManager.spawnSmoothAreas(config, chunkCoordinate, this.start, structureCache, worldGenRegion);
			...
			for (BO4CustomStructureCoordinate coordObject : objectsInChunk)
			{
				...
				BO4 bo4 = ((BO4)coordObject.getObject(otgRootFolder));
			...
			smoothingAreaManager.spawnSmoothAreas(config, chunkCoordinate, this.start, structureCache, worldGenRegion);
		}
