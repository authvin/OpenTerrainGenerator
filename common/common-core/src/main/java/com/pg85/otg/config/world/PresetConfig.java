 ...
 */
public class PresetConfig extends PresetSettings implements ConfigFile {
    ...
    public PresetConfig(Path settingsDir, SettingsMap settingsReader, ArrayList<String> biomes, IConfigFunctionProvider biomeResourcesManager, ILogger logger, IMaterialReader materialReader, String presetFolderName) {
        this.configName = settingsReader.getName();

        this.renameOldSettings(settingsReader, logger, materialReader);
        this.readConfigSettings(settingsReader, biomeResourcesManager, biomes, logger, materialReader, settingsDir);
    }
    ...
    protected void readConfigSettings(SettingsMap reader, IConfigFunctionProvider biomeResourcesManager, List<String> biomes, ILogger logger, IMaterialReader materialReader, Path settingsDir) {
        // Preset Info
        ...
        int generationDepth = reader.getSetting(PresetStandardValues.GENERATION_DEPTH, logger);
        int landSize = lowerThanOrEqualTo(reader.getSetting(PresetStandardValues.LAND_SIZE, logger), generationDepth);
        int landFuzzy = lowerThanOrEqualTo(reader.getSetting(PresetStandardValues.LAND_FUZZY, logger), generationDepth - landSize);
        int riverRarity = lowerThanOrEqualTo(reader.getSetting(PresetStandardValues.RIVER_RARITY, logger), generationDepth);
        int riverSize = lowerThanOrEqualTo(reader.getSetting(PresetStandardValues.RIVER_SIZE, logger), generationDepth - riverRarity);

        biomeSettingsBuilder.biomeRarityScale(reader.getSetting(PresetStandardValues.BIOME_RARITY_SCALE, logger));
        biomeSettingsBuilder.generationDepth(generationDepth);
        ...
        biomeSettingsBuilder.riverRarity(riverRarity);
        biomeSettingsBuilder.riverSize(riverSize);
        biomeSettingsBuilder.riversEnabled(reader.getSetting(PresetStandardValues.RIVERS_ENABLED, logger));

        String imageFile = reader.getSetting(PresetStandardValues.IMAGE_FILE, logger);
        BiomeMode biomeMode = reader.getSetting(PresetStandardValues.BIOME_MODE, logger);
        ...
        this.biomeGroupManager.filterBiomes(biomes, logger);

        biomeSettingsBuilder.isleBiomes(filterBiomes(reader.getSetting(PresetStandardValues.ISLE_BIOMES, logger), biomes));
        biomeSettingsBuilder.borderBiomes(filterBiomes(reader.getSetting(PresetStandardValues.BORDER_BIOMES, logger), biomes));
        ...
        int waterLevelMin = reader.getSetting(PresetStandardValues.WATER_LEVEL_MIN, logger);
        ...
        terrainSettingsBuilder.waterLevelMax(higherThanOrEqualTo(reader.getSetting(PresetStandardValues.WATER_LEVEL_MAX, logger), waterLevelMin));
        terrainSettingsBuilder.waterLevelMin(waterLevelMin);
        ...
        String imageFillBiome = reader.getSetting(PresetStandardValues.IMAGE_FILL_BIOME, logger);

        imageSettingsBuilder.imageOrientation(reader.getSetting(PresetStandardValues.IMAGE_ORIENTATION, logger));
        imageSettingsBuilder.imageFile(imageFile);
        ...
        int caveMinAltitude = reader.getSetting(PresetStandardValues.CAVE_MIN_ALTITUDE, logger);
        int caveSystemPocketMinSize = reader.getSetting(PresetStandardValues.CAVE_SYSTEM_POCKET_MIN_SIZE, logger);
        int ravineMinAltitude = reader.getSetting(PresetStandardValues.RAVINE_MIN_ALTITUDE, logger);
        int ravineMinLength = reader.getSetting(PresetStandardValues.RAVINE_MIN_LENGTH, logger);
        ...
        carverSettingsBuilder.caveMinAltitude(caveMinAltitude);
        carverSettingsBuilder.caveMaxAltitude(higherThanOrEqualTo(reader.getSetting(PresetStandardValues.CAVE_MAX_ALTITUDE, logger), caveMinAltitude));
        ...
        carverSettingsBuilder.caveSystemPocketMinSize(caveSystemPocketMinSize);
        carverSettingsBuilder.caveSystemPocketMaxSize(higherThanOrEqualTo(reader.getSetting(PresetStandardValues.CAVE_SYSTEM_POCKET_MAX_SIZE, logger), caveSystemPocketMinSize));
        ...
        carverSettingsBuilder.ravineMinLength(ravineMinLength);
        carverSettingsBuilder.ravineMaxLength(higherThanOrEqualTo(reader.getSetting(PresetStandardValues.RAVINE_MAX_LENGTH, logger), ravineMinLength));
        carverSettingsBuilder.ravineDepth(reader.getSetting(PresetStandardValues.RAVINE_DEPTH, logger));
        carverSettingsBuilder.ravineMinAltitude(ravineMinAltitude);
        carverSettingsBuilder.ravineMaxAltitude(higherThanOrEqualTo(reader.getSetting(PresetStandardValues.RAVINE_MAX_ALTITUDE, logger), ravineMinAltitude));
    ...
    private void readTemplateBiomes(SettingsMap reader, IConfigFunctionProvider biomeResourcesManager, ILogger logger, IMaterialReader materialReader) {
        ...
        for (ConfigFunction<PresetSettings> res : reader.getConfigFunctions((PresetSettings) this, biomeResourcesManager, logger, materialReader)) {
            if (res != null) {
    ...
    public void writeConfigSettings(SettingsMap writer) {
        ...
        writer.putSetting(PresetStandardValues.AMBIENT_LIGHT, this.getDimensionSettings().getAmbientLight(),
